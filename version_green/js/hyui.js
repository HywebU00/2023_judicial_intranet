$(function () {
  ...
  var liHasChilda = liHasChild.children('a');
  ...
  //         $(this).children('ul').find('ul').stop().slideUp(200);
  //         $('body').removeClass('fixed');
  //       });

  //     liHasChilda.on('click', function () {
  ...
  function tabSet2() {
    $('.announcement_tabs').each(function () {
      ...
      if (ww >= wwMedium) {
        ...
        _tabItem.outerWidth(tabItemWidth).css('margin-left', marginLeft);
        _tabItem.first().css('margin-left', 0);
        _tabItem.last().css({ position: 'absolute', top: 0, right: 10 }).outerWidth(tabItemWidth);
      } else {
        _tab.css('height', 'auto');
        _tabItem.width(tabwidth);
        _tabItem.css('margin-left', 0).last().css('position', 'relative');
